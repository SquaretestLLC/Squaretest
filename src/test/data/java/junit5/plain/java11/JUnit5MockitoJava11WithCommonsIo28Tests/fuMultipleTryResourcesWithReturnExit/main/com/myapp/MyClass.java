...
public class MyClass {
    ...
    public void doCopy1() {
        try (final InputStream inputStream = inputProvider.openInputStream(); final OutputStream outputStream = outputProvider.openOutputStream()) {
            if(inputStream == null || outputStream == null) {
                metricService.recordInputOrOutputEqualsNull();
                return;
            }
            IOUtils.copy(inputStream, outputStream);
            metricService.recordCopySuccessful();
        } catch (IOException e) {
