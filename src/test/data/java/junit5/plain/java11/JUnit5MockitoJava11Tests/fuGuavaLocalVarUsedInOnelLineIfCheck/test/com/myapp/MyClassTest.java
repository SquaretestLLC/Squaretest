...
class MyClassTest {
    ...
    @Test
    void testTryCreate8_ThrowsOrderAlreadyExistsException() {
        // Setup
        final Order order = new Order("orderId", "description");
        when(mockOrderService.getOrderWithId1("orderId")).thenReturn(new Order("orderId", "description"));

        // Run the test
        assertThrows(OrderAlreadyExistsException.class, () -> myClassUnderTest.tryCreate8(order));
    }

    @Test
    void testTryCreate8_OrderServiceGetOrderWithId1ReturnsNull() {
        // Setup
        final Order order = new Order("orderId", "description");
        when(mockOrderService.getOrderWithId1("orderId")).thenReturn(null);
        when(mockOrderService.create(any(Order.class))).thenReturn(new Order("orderId", "description"));
        ...
        // Run the test
        final Order result = myClassUnderTest.tryCreate8(order);
