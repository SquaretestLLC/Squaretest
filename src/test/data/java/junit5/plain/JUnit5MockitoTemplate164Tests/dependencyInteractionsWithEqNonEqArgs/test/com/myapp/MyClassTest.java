...
class MyClassTest {
    ...
    void testCallAllDIs() {
        // Setup
        when(mockFoo.doSomethingWithPrimitiveArgs(0, 1)).thenReturn(0);
        when(mockFoo.useNonEqualsOverridingArgs(any(Foo.Bar.class))).thenReturn(new Foo.Bar());
        when(mockFoo.useNonEqualsOverridingArgsWithNormalArgs(any(Foo.Bar.class), eq("asdf"), eq(1)))
                .thenReturn("result");
        when(mockFoo.useEqualsOverridingArg(new Foo.BarWithEquals())).thenReturn(new Foo.BarWithEquals());
        when(mockFoo.useEqualsOverridingArgSubtype(new Foo.SubBarWithEquals())).thenReturn(new Foo.BarWithEquals());
        doAnswer(invocation -> {
