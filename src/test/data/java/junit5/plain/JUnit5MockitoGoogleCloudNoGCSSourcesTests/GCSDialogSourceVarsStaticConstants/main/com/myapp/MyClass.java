...
public class MyClass {
    ...
    private final SpeechModelVariant speechModel;
    private final int sampleRateHertz;

    public MyClass(
            final SessionsClient sessionsClient,
            final SpeechModelVariant speechModel, final int sampleRateHertz) {
        this.sessionsClient = sessionsClient;
        this.speechModel = speechModel;
        this.sampleRateHertz = sampleRateHertz;
    }

    public void tryDetectIntent(final AudioEncoding audioEncoding, final double speakingRate) {
        DetectIntentRequest.Builder startingRequest = DialogHelpers.createRequest("theParam");
        final OutputAudioEncoding outputEncoding;
        if (audioEncoding == AudioEncoding.AUDIO_ENCODING_FLAC) {
            outputEncoding = OutputAudioEncoding.OUTPUT_AUDIO_ENCODING_MP3;
        } else {
            outputEncoding = OutputAudioEncoding.OUTPUT_AUDIO_ENCODING_MP3_64_KBPS;
        }
        final DetectIntentRequest request = startingRequest
                ...
                .setOutputAudioConfig(OutputAudioConfig.newBuilder()
                        .setAudioEncoding(outputEncoding)
