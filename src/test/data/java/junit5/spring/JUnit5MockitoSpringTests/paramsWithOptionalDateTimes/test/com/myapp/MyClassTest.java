...
class MyClassTest {
    ...
    void testGetNextEvent2() throws Exception {
        // Setup
        // Run the test and verify the results
        mockMvc.perform(get("/getNextEvent2/{pathDate}", "pathDate")
                        .param("requestVarDate", "requestVarDate")
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().json("{}", true));
    }
    ...
    void testGetNextEvent5() throws Exception {
        // Setup
        when(mockFooService.getTheDate("firstDate", "secondDate")).thenReturn("payload");

        // Run the test and verify the results
        mockMvc.perform(get("/getNextEvent5/{pathDate}", "pathDate")
                        .param("requestVarDate", "requestVarDate")
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().json("{}", true));
