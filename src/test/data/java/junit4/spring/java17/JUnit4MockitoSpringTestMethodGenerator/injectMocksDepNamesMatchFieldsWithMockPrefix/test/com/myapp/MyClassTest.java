...
public class MyClassTest {
    ...
    @Mock
    private ExecutorService mockExecutorService1;
    @Mock
    private ExecutorService mockExecutorService2;
    @Mock
    ...
    @Before
    public void setUp() {
        myClassUnderTest = new MyClass(mockUserServiceAdapter, mockExecutorService1, mockExecutorService2, mockLogger);
    }
    ...
    public void testGetAllUsersAsync1() {
        ...
        }).when(mockExecutorService1).submit((Callable<?>) any(Callable.class));
        when(mockUserServiceAdapter.getUsers()).thenReturn(List.of(new User()));
    ...
    public void testGetUserWithIdAsync1() {
        ...
        doAnswer(invocation -> {
            final Callable<?> callable = (Callable<?>) invocation.getArguments()[0];
            return CompletableFuture.completedFuture(callable.call());
        }).when(mockExecutorService2).submit((Callable<?>) any(Callable.class));
        when(mockUserServiceAdapter.getUserWithId("userId")).thenReturn(new User());
