...
public class SelectAllDepsAndMethodsInitInTestUiUtils implements Api.UiUtils {
    @Override
    ...
    public Map<String, Object> askUserToConfirmSettingsV2(final Map<String, Object> settingsToConfirm) {
        ...
        final boolean canCreateSourceInEachTest = Utils.safeGetCanCreateSourceClassInEachTestMethod(settingsToConfirm);
        if(canCreateSourceInEachTest) {
            settingsToConfirm.put("shouldCreateSourceClassInEachTestMethod", true);
        }
    ...
    private void updateVars(final List<? extends Api.Variable> variables) {
        for(final Api.Variable variable : variables) {
            if(canMock(variable)) {
                variable.setShouldBeMocked(true);
            }
        }
    }
}
