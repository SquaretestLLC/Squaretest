...
public class AWSUtilsValidateProcessor implements MethodCallProcessor {
    ...
    public AWSUtilsValidateProcessor(@NotNull final JavaPsiFacade javaPsiFacade, @NotNull final PsiClass sourceClass) {
    ...
    public NextStep tryProcessMethodCallWithArgThatContainsElement(
        ...
        if(!isValidateClass(containingClass)) {
            return null;
        }
        ...
        if(methodName.equals("validState")) {
            if(index != 0 || hasPredicateParam(psiMethod)) {
                return null;
            }
            if(currentReturnOutcome == ReturnOutcome.False) {
                ret.addElementsContainingDisHit(getArgsThatDoNotContainElement(args, originalStartingElement));
    ...
    private static boolean hasPredicateParam(final PsiMethod psiMethod) {
        for(final PsiParameter parameter : psiMethod.getParameterList().getParameters()) {
            if(InheritanceUtil.isInheritor(parameter.getType(), "java.util.function.Predicate")) {
                return true;
            }
        }
        return false;
    ...
    private static boolean isValidateClass(final PsiClass containingClass) {
        final String qualifiedName = containingClass.getQualifiedName();
        return StringUtils.equalsAny(qualifiedName, "software.amazon.awssdk.utils.Validate");
    }
}
