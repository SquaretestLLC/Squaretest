...
public class ApacheCollectionUtilsProcessor implements MethodCallProcessor {
    @Override
    @Nullable
    public NextStep tryProcessMethodCallWithArgThatContainsElement(
        ...
        if(!isCollectionUtils(containingClass)) {
            return null;
        }
        ...
        if(StringUtils.equals(methodName, "isEmpty")) {
    ...
    private boolean isCollectionUtils(final PsiClass containingClass) {
        if(containingClass == null) {
            return false;
        }
        return StringUtils.equals(containingClass.getQualifiedName(), "org.apache.commons.collections4.CollectionUtils");
    }
}
