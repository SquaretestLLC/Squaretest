...
public class ApacheCommonsLangValidateProcessor implements MethodCallProcessor {
    ...
    public ApacheCommonsLangValidateProcessor(@NotNull final JavaPsiFacade javaPsiFacade, @NotNull final PsiClass sourceClass) {
    ...
    public NextStep tryProcessMethodCallWithArgThatContainsElement(
        ...
        if(methodName.equals("notEmpty")) {
            ...
            if(currentReturnOutcome == ReturnOutcome.Empty || currentReturnOutcome == ReturnOutcome.Null) {
                ret.addElementsContainingDisHit(getArgsThatDoNotContainElement(args, originalStartingElement));
                ret.withExceptionThrown(currentMethodCall, getIllegalArgumentException());
                return NextStep.Return;
            }
    ...
    private static boolean isValidateClass(final PsiClass containingClass) {
        ...
        return StringUtils.equalsAny(qualifiedName, "org.apache.commons.lang.Validate");
    }
}