...
public class JavaArraysProcessor implements MethodCallProcessor {
    @Override
    @Nullable
    public NextStep tryProcessMethodCallWithArgThatContainsElement(
        ...
        if(!isArraysClass(containingClass)) {
            return null;
        }
        ...
        if(StringUtils.equalsAny(methodName, "stream", "spliterator")) {
            if(index != 0) {
                return null;
            }
    ...
    private boolean isArraysClass(final PsiClass containingClass) {
        if(containingClass == null) {
            return false;
        }
        return StringUtils.equals(containingClass.getQualifiedName(), "java.util.Arrays");
    }
}
